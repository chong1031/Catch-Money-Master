...
NSString *highscorelist,*selection,*arrowstring=@"menu_button4",*textstring=@"menu_shop";
...
+(id)scene{
    ...
    NSLog(@"windows size = %f", [UIScreen mainScreen].bounds.size.height);
    return [shared sharedInstance].mainScene;
}
+(id)scene1{
    [shared sharedInstance].mainScene=[[self alloc] initWithContentOfFile:@"final/my1.lhplist"];
    NSLog(@"windows size = %f", [UIScreen mainScreen].bounds.size.height);
    return [shared sharedInstance].mainScene;
}
-(id)initWithContentOfFile:(NSString *)levelPlistFile{
    ...
    if(self = [super initWithContentOfFile:levelPlistFile])
    {
        ...
        arrowstring=@"menu_button4";
        textstring=@"menu_shop";
...
//            [self setScale:my.frame.size.height/960];
//            
//        }
    }
...
-(void)movetoachievement{
    ...
    [_tableView setFrame:CGRectMake(CGRectGetMidX(_temptableView.frame)/4, -1.2*CGRectGetMidY(_temptableView.frame)-(currentlc.y-        currentLocation.y)/2,_temptableView.frame.size.width ,_temptableView.frame.size.height-50)];
...
//    NSLog(@"%@",arrayValues);
    ...
    if([shared sharedInstance].flag==1) {
        ...
        if(init_3d)[self movetogameplay];
...
-(void)maketable {
    NSLog(@"size.fame.size = %f %f", self.frame.size.width, self.frame.size.height);
    if(self.frame.size.height==1334){//iphone6
        ...
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(CGRectGetMinX(self.frame)+(self.frame.size.width/13)+5,CGRectGetMaxY(self.frame)/5, self.frame.size.width/2-13, self.frame.size.height/2.72)];
    }
    else if (self.frame.size.height==1136){ //iphone5
        ...
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(CGRectGetMinX(self.frame)+(self.frame.size.width/15)+self.frame.size.width*0.05/2.8,CGRectGetMaxY(self.frame)/7, self.frame.size.width*0.9/2.8, self.frame.size.height*0.95/3.2)];
    }
    else{
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(CGRectGetMinX(self.frame)+(self.frame.size.width/15)-5,CGRectGetMaxY(self.frame)/7+3, self.frame.size.width/2.7, self.frame.size.height/3.4)];
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (outerlock == 0) {
        if ([touchedNode.name isEqualToString:@"menu_button1"] || [touchedNode.name isEqualToString:@"menu_quickPlay"] || [touchedNode.name isEqualToString:@"menu_button3"] || [touchedNode.name isEqualToString:@"menu_storyMode"]) {
        ...
        if ([touchedNode.name isEqualToString:@"menu_button4"] || [touchedNode.name isEqualToString:@"menu_shop"] || [touchedNode.name isEqualToString:@"menu_button2"] || [touchedNode.name isEqualToString:@"menu_achievements"]) {
            NSLog(@"move vertical");
            fmove=0;
            flag_i=1;
            locked=0;
    ...
    if(shopbuttonscroll==0) {
        if(screen_identifier==0) {
            ...
            if(locked==0){
                if(flag_i==1){
                    if([touchedNode.name isEqualToString:@"menu_shop"] || [touchedNode.name isEqualToString:@"menu_button4"]){
                        NSLog(@"down");
                        textstring=[NSString stringWithFormat:@"menu_shop"];
                        arrowstring=[NSString stringWithFormat:@"menu_button4"];
                    ...
                    if([touchedNode.name isEqualToString:@"menu_achievements"] || [touchedNode.name isEqualToString:@"menu_button2"]){
                        NSLog(@"up");
                        textstring=[NSString stringWithFormat:@"menu_achievements"];
                        arrowstring=[NSString stringWithFormat:@"menu_button2"];
            ...
            if(locked==1){
                if(flag_i==2){
                    // from left to right
                    if([touchedNode.name isEqualToString:@"menu_storyMode"] || [touchedNode.name isEqualToString:@"menu_button3"]){
                ...
                if([touchedNode.name isEqualToString:@"menu_quickPlay"] || [touchedNode.name isEqualToString:@"menu_button1"]){
                    NSLog(@"right");
                    textstring=[NSString stringWithFormat:@"menu_quickPlay"];
                    arrowstring=[NSString stringWithFormat:@"menu_button1"];
...
-(NSString*)textureAtlasNamed:(NSString *)fileName{
    ...
    if(self.frame.size.height==1334){//iphone6
        ...
        fileName =fileName;
//        fileName = [NSString stringWithFormat:@"%@-480", fileName];
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (speed >=400) {
        ...
        if(customswipe){
            ...
            if(currentlc.y-currentLocation.y>=self.frame.size.height/2 && firstmove==0){
            ...
            else if(currentLocation.y-currentlc.y>=self.frame.size.height/2 && firstmove==0){
            ...
            else if(currentlc.x-currentLocation.x>=self.frame.size.width/2 && firstmove==1){
            ...
            else if(currentLocation.x-currentlc.x>=self.frame.size.width/2 && firstmove==1){
...
-(void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    NSLog(@"touch location : %f %f", currentlc.x, currentLocation.x);
    
    if(outerlock==0){
        instantlocation=[[touches anyObject]locationInNode:self];
        if(( (currentlc.y>instantlocation.y) || (currentlc.y<instantlocation.y)) && (fabs(instantlocation.y-currentlc.y) > fabs(instantlocation.x-currentlc.x)) ){
            NSLog(@"move vertical");
            fmove=0;
            flag_i=1;
            locked=0;
        ...
        else if( ((currentlc.x>instantlocation.x) || (currentlc.x<instantlocation.x)) && (fabs(instantlocation.x-currentlc.x) > fabs(instantlocation.y-currentlc.y)) ){
            NSLog(@"move horizontal");
            ...
            if(fmove==0){
                if(currentlc.y>instantlocation.y){
                    ...
                    if(currentLocation.y<currentlc.y){
                        ...
                        //second.position=CGPointMake(CGRectGetMidX(self.frame),CGRectGetMidY(self.frame)-((man.frame.size.height)-(currentLocation.y-currentlc.y)));
                        [_tableView setFrame:CGRectMake(CGRectGetMidX(_tableView.frame)/4,CGRectGetMidY(_tableView.frame)/2-(currentlc.y-currentLocation.y)/4,_tableView.frame.size.width ,_tableView.frame.size.height-50)];
